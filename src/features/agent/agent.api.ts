...
export async function sendAgentChatMessage({
  ...
  const contextText = JSON.stringify({
    currentForm: {
      ...
      nanoBananaTemperature: formSnapshot.nanoBananaTemperature,
      nanoBananaTopP: formSnapshot.nanoBananaTopP,
      nanoBananaMaxTokens: formSnapshot.nanoBananaMaxTokens,
      nanoBananaSeed: formSnapshot.nanoBananaSeed,
    },
    templates: templates.slice(0, 30).map((template) => ({
      ...
      nanoBananaTemperature: template.nanoBananaTemperature,
      nanoBananaTopP: template.nanoBananaTopP,
      nanoBananaMaxTokens: template.nanoBananaMaxTokens,
      ...
      input: [
        {
          role: 'system',
          content:
            '你是个人 AI 生图控制台里的私人创作 Agent。你只能围绕对方自己的生图、图生图、提示词、参数、个人模板、我的历史结果、上传图片分析和失败排查提供帮助。对方可能会上传图片供你分析画面内容、风格、构图、色彩、主体和可复用提示词。必须只输出严格 JSON，不要 Markdown。JSON 字段为 reply、proposedActions。reply 是中文自然回复，使用面向个人用户的口吻。proposedActions 是数组，可以为空。动作类型只能是 formPatch、generate、applyTemplate、showHistoryResult、createTemplate、explain。所有会改变表单、应用模板、展示历史结果、创建模板或生图的意图都必须作为 proposedActions 给出，由对方确认后执行；不要声称已经执行。formPatch.payload.formPatch 只允许包含 mode、imageModelFamily、imageModel、prompt、negativePrompt、aspectRatio、resolutionTier、quality、moderation、count、compressionRate、outputFormat、nanoBananaTemperature、nanoBananaTopP、nanoBananaMaxTokens、nanoBananaSeed。imageModelFamily 只能是 gpt-image-2、nano-banana-2、nano-banana-pro；imageModel 只能是 gpt-image-2、nano-banana-2、nano-banana-2-1K、nano-banana-2-2K、nano-banana-2-4K、nano-banana-pro、nano-banana-pro-1K、nano-banana-pro-2K、nano-banana-pro-4K。nanoBananaTemperature 范围 0 到 2；nanoBananaTopP 范围 0 到 1；nanoBananaMaxTokens 必须大于等于 1；nanoBananaSeed 是整数，可省略。generate 可以带 payload.formPatch 表示生成前要应用的表单改动。createTemplate 用于把当前表单或建议表单保存成对方的个人模板，payload 可包含 name、description、formPatch。applyTemplate 必须带 templateId。showHistoryResult 必须带 historyId。explain 是纯说明动作，不应改变状态。',
        },
        {
          role: 'user',
          content: [
